...
class QuantumImageEncoder:
    ...
    def measure(self) -> Dict[str, int]:
        aer_sim = Aer.get_backend('aer_stimulator')
        self.qc.measure_all()
    ...
    def measure_x_reduced(self) -> Tuple[Dict[str, int], int, int]:
        ...
        result = job.result()
        counts = result.get_counts()
        return {k: v for k, v in counts.items() if k[0] == '1'}, qc.size(),qc.depth(), 
...
def validate_counts(counts: Dict[str, int], image: List[float]) -> List[str]:
    ...
    for k in counts:
        idx = int(k[1:11], 2)
        intensity = int(k[11:], 2)
        expected = int(image[idx])
        # print(idx, intensity, expected, intensity==expected)
